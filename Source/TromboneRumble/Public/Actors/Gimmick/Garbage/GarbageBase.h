...
protected:
	...
	UPROPERTY(EditAnywhere, Category = "Garbage|Config|Throw", meta = (GimmickSetting, DisplayName = "최소 추가 정점 높이"))
	float MinExtraApexHeight = 150.f;

	/** 투척 시 최대 추가 높이 (cm) 
	 * 시작점과 목표점 중 높은 곳을 기준으로, 최대한 이 값까지 위로 솟구칠 수 있습니다. */
	UPROPERTY(EditAnywhere, Category = "Garbage|Config|Throw", meta = (GimmickSetting, DisplayName = "최대 추가 정점 높이"))
	float MaxExtraApexHeight = 450.f;

	/** 초당 최소 회전 속도 (도/sec) 
	 * 날아가는 동안 무작위 축으로 회전할 최소 속도입니다. */
	UPROPERTY(EditAnywhere, Category = "Garbage|Config|Throw", meta = (GimmickSetting, DisplayName = "최소 회전 속도"))
	float MinSpinDegPerSec = 180.f;

	/** 초당 최대 회전 속도 (도/sec) 
	 * 날아가는 동안 무작위 축으로 회전할 최대 속도입니다. */
	UPROPERTY(EditAnywhere, Category = "Garbage|Config|Throw", meta = (GimmickSetting, DisplayName = "최대 회전 속도"))
	float MaxSpinDegPerSec = 900.f;

	/** 착지 후 제거 대기 시간 (초) 
	 * 바닥(WorldStatic/Dynamic)에 닿은 후 액터가 파괴될 때까지의 시간입니다. */
	UPROPERTY(EditAnywhere, Category = "Garbage|Config|Lifetime", meta = (GimmickSetting, DisplayName = "착지 후 제거 지연"))
	float DestroyDelayAfterLand = 5.0f;

	/** 충돌 후 제거 대기 시간 (초) 
	 * 캐릭터 등과 충돌한 후 액터가 파괴될 때까지의 시간입니다. */
	UPROPERTY(EditAnywhere, Category = "Garbage|Config|Lifetime", meta = (GimmickSetting, DisplayName = "충돌 후 제거 지연"))
	float DestroyDelayAfterImpact = 2.0f;
	
	/** 피해 유형 
	 * 이 투척물이 캐릭터와 충돌했을 때 적용할 피해 유형입니다. */
	UPROPERTY(EditAnywhere, Category = "Garbage|Config|HitType", meta = (GimmickSetting, DisplayName = "피해 유형"))
	EHitReactionType HitReactionType = EHitReactionType::None;

	/** 수평 넉백 힘
	 * 충돌한 캐릭터를 수평으로 밀어내는 힘입니다. 피해 유형(스턴/래그돌)에 맞게 튜닝하세요. */
	UPROPERTY(EditAnywhere, Category = "Garbage|Config|HitType", meta = (GimmickSetting, DisplayName = "넉백 힘 (수평)", EditCondition = "HitReactionType != EHitReactionType::None"))
	float KnockbackForce = 500.f;

	/** 수직 넉백 힘
	 * 충돌한 캐릭터를 위로 띄우는 힘입니다. 피해 유형(스턴/래그돌)에 맞게 튜닝하세요. */
	UPROPERTY(EditAnywhere, Category = "Garbage|Config|HitType", meta = (GimmickSetting, DisplayName = "넉백 힘 (수직)", EditCondition = "HitReactionType != EHitReactionType::None"))
