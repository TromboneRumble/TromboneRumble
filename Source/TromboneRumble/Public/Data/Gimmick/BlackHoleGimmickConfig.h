...
public:
	...
	/** Speed toward the center. 1 at the edge means nobody can walk out. */
	UPROPERTY(EditAnywhere, Category = "Orbit", meta = (DisplayName = "거리별 끌림 비율", XAxisName = "거리 비율", YAxisName = "끌림 비율"))
	FRuntimeFloatCurve PullCurve;

	/** Sideways speed that turns the pull into an orbit. Same axes as the pull curve. */
	UPROPERTY(EditAnywhere, Category = "Orbit", meta = (DisplayName = "거리별 공전 비율", XAxisName = "거리 비율", YAxisName = "공전 비율"))
	...
	/** Speed the curves scale for props and ragdolls. A walking player uses their own walk speed instead. */
	UPROPERTY(EditAnywhere, Category = "Physics", meta = (DisplayName = "소품/래그돌 기준 속도", ClampMin = "0.0", Units = "cm/s"))
	...
	/** Speed the captured things leave with when the hole collapses. */
	UPROPERTY(EditAnywhere, Category = "Burst", meta = (DisplayName = "방출 속도", ClampMin = "0.0", Units = "cm/s"))
