	...
	const FWwiseLanguageCookedData& InLanguage, FLoadExternalSourceCallback&& InCallback)
{
	...
	("FWwiseExternalSourceManagerImpl::LoadExternalSource"), [WeakThis=AsWeak(), InExternalSourceCookedData, InRootPath, InLanguage, InCallback = MoveTemp(InCallback)]() mutable
	{
		...
		SharedExternalSourceManager->LoadExternalSourceImpl(InExternalSourceCookedData, InRootPath, InLanguage, MoveTemp
		(InCallback));
	});
}
	...
	const FWwiseLanguageCookedData& InLanguage, FUnloadExternalSourceCallback&& InCallback)
{
	...
	FileHandlerExecutionQueue->Async(WWISEFILEHANDLER_ASYNC_NAME
	("FWwiseExternalSourceManagerImpl::UnloadExternalSource"), [WeakThis=AsWeak(), InExternalSourceCookedData, InRootPath, InLanguage, InCallback = MoveTemp(InCallback)]() mutable
	{
		auto SharedExternalSourceManager = StaticCastSharedPtr<FWwiseExternalSourceManagerImpl>(WeakThis.Pin());
		if (!SharedExternalSourceManager.IsValid())
		{
		...
		SharedExternalSourceManager->UnloadExternalSourceImpl(InExternalSourceCookedData, InRootPath, InLanguage, MoveTemp
		(InCallback));
	});
}
		...
		for (const auto MediaId : InMediaIds)
		{
			...
			("FWwiseExternalSourceManagerImpl::BindPlayingIdToExternalSources decrement"), [WeakThis=AsWeak(), MediaId, ExternalSourceFileState]() mutable
			{
				...
				if (ExternalSourceFileState->DecrementPlayCount() && ExternalSourceFileState->CanDelete())
				{
					...
					SharedExternalSourceManager->OnDeleteState(MediaId, *ExternalSourceFileState, 
					EWwiseFileStateOperationOrigin::Loading, []{});
				}
			});
		}
	}
