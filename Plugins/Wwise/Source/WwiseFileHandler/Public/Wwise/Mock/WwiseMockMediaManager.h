...
public:
	...
	void LoadMedia(const FWwiseMediaCookedData& InMediaCookedData, const FString& InRootPath, FLoadMediaCallback&& InCallback) override
	{
		...
		IncrementFileStateUseAsync(InMediaCookedData.MediaId, EWwiseFileStateOperationOrigin::Loading, [WeakThis=AsWeak(), InMediaCookedData, InRootPath]() mutable
		{
			...
			return SharedMockMediaManager->CreateOp(InMediaCookedData, InRootPath);
