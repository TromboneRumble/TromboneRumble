	...
	const FWwiseSharedPlatformId& InPlatform, const FWwiseSharedLanguageId& InLanguage)
{
	...
	MediaInfoTable->ForeachRow<FWwiseExternalSourceMediaInfo>(Context, [this, SourceDirectory, &InResourceCooker, &WriteAdditionalFile](const FName& Key, const FWwiseExternalSourceMediaInfo& MediaInfo)
	{
	...
	FLoadExternalSourceCallback&& InCallback)
{
	...
	IncrementFileStateUse(MediaId, EWwiseFileStateOperationOrigin::Loading,
		[WeakThis=AsWeak(), MediaId, &InRootPath]() mutable -> FWwiseFileStateSharedPtr
	{
		...
		{
			return SharedSimpleExtSrcManager->CreateOp(*ExternalSourceMediaInfoEntry, InRootPath);
		}
		else
		{
...
void FWwiseSimpleExtSrcManager::SetExternalSourceMedia(const uint32 ExternalSourceCookie, const uint32 MediaInfoId, const FName& ExternalSourceName)
{
	...
	ExternalSourceName, &Completed]() mutable
	{
		...
		if (ExternalSourceLoadCount)
		{
			...
			for (int i = ExternalSourceLoadCount-1; i >= 0; --i)
			{
				if (i == 0)
				{
					...
					SharedSimpleExtSrcManager->UnloadExternalSourceMedia(ExternalSourceCookie, ExternalSourceName, FWwiseResourceLoader::Get()->GetUnrealExternalSourcePath(),
		...
		for (int i = ExternalSourceLoadCount-1; i >= 0; --i)
		{
			if (i == 0)
			{
				SharedSimpleExtSrcManager->LoadExternalSourceMedia(ExternalSourceCookie, ExternalSourceName, FWwiseResourceLoader::Get()->GetUnrealExternalSourcePath(), [&Completed, UnloadFuture = MoveTemp(UnloadFuture)](bool)
				{
					UnloadFuture.Wait();
					Completed->Trigger();
				});
			...
			{
				SharedSimpleExtSrcManager->LoadExternalSourceMedia(ExternalSourceCookie, ExternalSourceName, FWwiseResourceLoader::Get()->GetUnrealExternalSourcePath(), [&Completed](bool) {});
			}
		}
	});
